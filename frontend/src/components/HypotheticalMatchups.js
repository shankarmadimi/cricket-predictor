...
const HypotheticalMatchups = () => {
  ...
  const handlePlayerSimulation = async (e) => {
    e.preventDefault();
    if (!player1 || !player2) {
      setResult({ message: "Please select both players." });
      return;
    }
    setLoading(true);
    setResult(null);
  ...
  return (
    <section className="bg-gray-700 rounded-2xl p-6 shadow-lg">
      ...
      <form onSubmit={handlePlayerSimulation} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <button
          type="submit"
          disabled={loading}
          className="col-span-1 sm:col-span-2 mt-4 bg-blue-500 hover:bg-blue-600 text-white font-bold py-3 rounded-xl transition duration-300 ease-in-out transform hover:scale-105 shadow-md disabled:bg-gray-500 disabled:cursor-not-allowed"
        >
